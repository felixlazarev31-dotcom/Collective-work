...
class Message(db.Model):
    ...
    timestamp = db.Column(db.DateTime, default=datetime.utcnow) # Смотрит на Гринвич
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
...
def index():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    todo_list = Todo.query.filter_by(user_id=session['user_id']).order_by(Todo.complete.asc(), Todo.due_date.asc()).all()
    # Загружаем последние 50 сообщений чата
    chat_messages = Message.query.order_by(Message.timestamp.asc()).all()[-50:]
    return render_template('index.html', todo_list=todo_list, username=session['username'], chat_messages=chat_messages)
